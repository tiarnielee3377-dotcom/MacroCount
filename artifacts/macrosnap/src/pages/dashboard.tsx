...
import { readDashboardSnapshot, saveDashboardSnapshot } from "@/lib/offline-dashboard";
import { syncDailyStreakReminder } from "@/lib/native";
...
export default function Dashboard() {
  ...
  const [cachedSnapshot] = useState(() => readDashboardSnapshot(timeZone));
  ...
  const hasCachedDashboard = Boolean(cachedSnapshot?.profile);
  const profile = profileData ?? cachedSnapshot?.profile;

  const {
    data: mealsData,
    isLoading: isMealsLoading,
    isError: isMealsError,
  } = useListMeals({ date, timeZone }, {
    query: {
  ...
  const { data: progressData, isError: isProgressError } = useGetProgress({ timeZone }, {
    query: {
  ...
  const { data: workoutSummaryData, isError: isWorkoutError } = useGetWorkoutSummary({ timeZone }, {
    query: { queryKey: getGetWorkoutSummaryQueryKey({ timeZone }), enabled: !!profile, retry: false },
  });
  const meals = mealsData ?? (hasCachedDashboard ? cachedSnapshot?.meals : undefined);
  const progress = progressData ?? (hasCachedDashboard ? cachedSnapshot?.progress ?? undefined : undefined);
  const workoutSummary =
    workoutSummaryData ?? (hasCachedDashboard ? cachedSnapshot?.workoutSummary ?? undefined : undefined);
  const isOfflineFallback = Boolean(
    hasCachedDashboard &&
      (!profileData || isMealsError || isProgressError || isWorkoutError),
  );
  const mealsAreLoading = isMealsLoading && !meals;
  ...
  }, [date, mealsData, profileData, progressData, timeZone, workoutSummaryData]);

  useEffect(() => {
    if (!progress || !meals) return;
    void syncDailyStreakReminder({
      hasActiveStreak: progress.streak.current > 0 && !progress.streak.isBroken,
      hasLoggedToday: meals.length > 0,
    });
  }, [meals, progress]);
  ...
  if (isProfileLoading && !hasCachedDashboard) {
    return (
      <MobileLayout>
        ...
        <header className="pt-8 pb-6 px-5">
          <div className="flex justify-between items-center">
            ...
            {isOfflineFallback && (
              <div
                role="status"
                className="mt-4 rounded-2xl border border-[#FFB020]/25 bg-[#FFB020]/10 px-4 py-3 text-sm font-medium text-[#9A6400]"
              >
                You’re offline — showing your last synced dashboard.
              </div>
            )}
          {mealMomentMessage && (
